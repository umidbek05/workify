...
const DashboardHome = () => {
  ...
  const CustomTabSwitcher = () => (
    <div
      className={`flex p-1 rounded-[14px] mt-4 self-center`}
      style={{ backgroundColor: themeStyles.switcherBg }}
    >
      {["week", "month"].map((tab) => (
        <button
          key={tab}
          ...
          style={{
            backgroundColor:
              activeTab === tab ? themeStyles.tabActive : "transparent",
            color: activeTab === tab ? themeStyles.text : themeStyles.subtext,
            boxShadow:
              activeTab === tab ? "0 4px 10px rgba(0,0,0,0.06)" : "none",
          }}
        >
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div
          className="w-full md:flex-1 h-[56px] rounded-[15px] flex items-center px-6 border shadow-sm"
          style={{
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6">
          ...
          >
            ...
            <div className="relative flex justify-center items-center my-5">
              ...
              >
                <div
                  className="w-[155px] h-[155px] rounded-full flex flex-col justify-center items-center"
                  style={{
                    background:
          ...
          {/* 2. PROFILE VIEWS */}
          <div
            className="rounded-[24px] p-6 flex flex-col h-[400px] border shadow-sm"
            style={{
              backgroundColor: themeStyles.card,
              borderColor: themeStyles.border,
            }}
            ...
            <div className="flex-1 w-full mt-8" style={{ minHeight: 0 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={activeViewsData} margin={{ left: -25 }}>
                  <CartesianGrid
                    vertical={false}
                    stroke={isDarkMode ? "#2a2a2a" : "#F4F7FE"}
                  />
                  <XAxis
                    dataKey="day"
                    axisLine={false}
...
const CustomTooltip = ({ active, payload, label, isDarkMode, themeStyles }) => {
  if (active && payload && payload.length) {
    return (
      <div
        className="p-2 shadow-xl rounded-lg border"
        style={{
